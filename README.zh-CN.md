...
- 自动书签同步：登录后打开“X 浏览器”即开始慢速同步；每次滚动都会等待实际书签分页响应确认，切到本 App 其他栏目后仍可继续。
...
2. 登录完成后 App 会自动打开书签并慢速同步；也可随时在原生 **书签** 标签点击 **同步** 再运行一轮。
3. 在书签页按账号、帖子或 Hashtag 浏览，并按 @用户名、显示昵称、数字 User ID、正文、Hashtag 或发布时间组合筛选。
4. 选择图片、动图、视频和可选日期范围。
5. 点击 **批量下载并保存到照片**。
...
- 自动同步不是按屏幕 cell 数盲目计时：每次滚动后最多等待 5 秒，捕获到真实分页响应才继续；连续 4 次没有响应才停止。这能明显降低快速滚动漏页风险，但无法让未公开文档的网页流程提供服务端全量保证。
