...
- 分类与搜索：只保留账号和 Hashtag 两种聚合；可搜索 @用户名、显示昵称、数字 User ID、正文和 Hashtag，并按账号帖子数或名称排序。
- 原生 Post 详情与保存：点击列表即可查看完整正文、原图和最高质量 MP4 预览，并把该 Post 去重后的媒体直接保存到系统照片。
- 已索引 Post 时间线：点击“已索引 Post”统计即可进入实时观察本地索引的连续媒体流，支持最新/最早排序、搜索，以及可记忆的纯文字切换。
...
Post 内的视频预览提供明确的全屏按钮；在小播放器上双指向外放大，也会打开全屏播放器。
